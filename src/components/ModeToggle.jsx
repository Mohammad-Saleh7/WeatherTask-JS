...
export default function ModeToggle() {
  ...
  return (
    <ButtonGroup
      ...
      variant="outlined"
      fullWidth
      sx={{ flexDirection: i18n.language === "fa" ? "row-reverse" : "row" }}
    >
      <Button onClick={() => setMode("light")} sx={{ display: "flex", gap: 1 }}>
        <LightModeOutlinedIcon sx={{ fontSize: "18px" }} /> {t("header.light")}
      </Button>
      <Button onClick={() => setMode("dark")} sx={{ display: "flex", gap: 1 }}>
        <BedtimeOutlinedIcon sx={{ fontSize: "18px" }} /> {t("header.dark")}
      </Button>
    </ButtonGroup>
  );
}
